define(function (require) {
    ...
    //$(document).on("click",".isUserBox", function () {
        ...
        if(old_val != new_val){
            $.ajax({
                ...
                success: function(msg){
                    ...
                    $("#props_show").remove();
                    $("#is_here").before(msg);
                    $(".select:gt(3)").select({onlyShow:false});//下拉

                    var f_val = $("#box_main").text();
                    if(f_val != ''){
    ...
    $.ajax({
        type: "get",
        url: "/index.php/Goods/index/getGoodsNum",
        data: '',
        success: function(msg) {
           $("#goods_num").val(msg);
            $("#goods_bn").val(msg+'0101');
        }
    });
    var isGoodsNumExist=false;

    $("#goods_num").blur(function(){
        var goods_num = $(this).val();
        $.ajax({
            type: "get",
            url: "/index.php/Goods/index/checkGoodsNum",
            data: 'goods_num='+goods_num,
            success: function(msg) {
                if(msg == '1'){
                    alert('商品编号已存在');
                    isGoodsNumExist=true;
                }else{
                    $("#goods_bn").val(goods_num+'0101');
                    isGoodsNumExist=false;
                }
            }
        });
    });
    var isGoodsBnExist=false;
    $("#goods_bn").blur(function(){
        var goods_num = $(this).val();
        $.ajax({
            type: "get",
            url: "/index.php/Goods/index/checkGoodsBn",
    ...
    $("#is_father").delegate(".checkbox","click", function () {
        ...
        var index = $(this).parents(".goodsClassify").index()-1;console.log(index);
        ...
        var listLength = $('#box_main').find('.goodsClassify').length;
        if($(this).hasClass("checked")){
            var tpl = false;
...
function addStr(str,val_1,val_id_1,val_2,val_id_2,val_3,val_id_3){
    ...
    if(n==1){
        str += '<td><input type="hidden" name="is_default[]" value="1"/><i class="checkbox isDefaultBox checked" v=""></i></td>';
    }else{
