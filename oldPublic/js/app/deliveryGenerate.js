...
define(function (require,exports,module) {
    ...
    $.getAddress = function(doc, dom, type) {
        $.post('/index.php/Delivery/Delivery/getAddress', {type: type, id: doc.value}, function (info) {
            info = $.parseJSON(info);
    ...
    //    $.post('/index.php/Delivery/Delivery/generate',$('form').serialize(),function(info){
    ...
    $('.gen-sureBtn').click(function() {
        ...
        if(result){
            //$('form').submit();//表单提交
            $.post('/index.php/Delivery/Delivery/generate',$('form').serialize(),function(info){//ajax提交
                var data = $.parseJSON(info);
                if(data.status == false){
                    alert(data.info);
                }else{
